class BinarySearchSorted2DArray
{
    public static void main(String[] args) 
    {
         ...
         * The lower bound is the first element in the first row, the upper bound is the topmost element in the last column.
         * Row count starts from 0, Column count starts from array[row].length - 1.
         * If the target element is less than the current array[row][column], then we discard the row (row++).
         * If the target element is greater than the current array[row][column], then we discard the column (column--).
         */
